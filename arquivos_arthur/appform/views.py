...
from .forms import saneform                   ########################## importa o formulario  Dogform definido no arquivo forms
from appform.models import sane               ######################### importa o modelo de dados Dogs  definido no arquivo models
...
def add_dog(request):
    ...
    if request.method == 'POST': ######################## verifica se a requisição utiliza o método http POST
        ...
        # form = saneform(updated_data, request.FILES) ######################## instancio o formulario com as informações modificadas e com os arquivos upados
        # updated_data = request.POST.copy() ######################## faço uma cópia das informações passadas no request
